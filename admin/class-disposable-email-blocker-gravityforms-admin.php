...
class Disposable_Email_Blocker_Gravityforms_Admin {
	...
	public function add_plugin_action_links( $links ) {
		$links[] = sprintf( '<a href="%s">%s</a>', esc_url( admin_url( 'admin.php?page=gf_edit_forms' ) ), __( 'Settings', 'disposable-email-blocker-gravityforms' ) );
	...
	public function admin_notices() {
		...
		if ( ! class_exists( 'GFForms', false ) ) {
			sprintf(
				...
				__( 'Disposable Email Blocker - GravityForms requires Gravity Forms plugin to be active!', 'disposable-email-blocker-gravityforms' ),
			);
		...
		$current_db_version = get_option( 'debgform_db_version', false );

		if ( DISPOSABLE_EMAIL_BLOCKER_GRAVITYFORMS_PLUGIN_DB_VERSION !== $current_db_version ) {
			...
			update_option( 'debgform_db_version', DISPOSABLE_EMAIL_BLOCKER_GRAVITYFORMS_PLUGIN_DB_VERSION );
		}
	}
